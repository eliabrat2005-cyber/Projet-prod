...
#
# Endpoints `/api/v1/*` pour le client iPhone/iPad — auth Bearer token,
# scopés au tenant_id du user. Le code des endpoints + le helper d'auth
# vivent dans `common/mobile_v1.py` ; on les enregistre ici sur l'app
# NiceGUI/FastAPI au boot.
#
# Le préfixe `/api/v1/` est listé dans PUBLIC_PATHS (en haut de ce fichier)
# pour bypasser le middleware d'auth web (cookies session). Chaque endpoint
# mobile vérifie ensuite son propre Bearer token.

from common.mobile_v1 import register_routes as _register_mobile_v1_routes

_register_mobile_v1_routes(app)
